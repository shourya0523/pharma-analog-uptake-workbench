...
REGISTER = {
    "uthr:NebulizedTyvasoMember": Resolution(
        "uthr:NebulizedTyvasoMember", "Nebulized Tyvaso", "exact", 1.0),
    "uthr:TyvasoDPIMember": Resolution(
...
def test_a_tagged_fact_becomes_a_candidate_with_its_period_and_unit():
    found, notes = candidates_from_instance(
        INSTANCE, product="Nebulized Tyvaso", register=REGISTER)
    assert len(found) == 1
    candidate = found[0]
...
def test_a_sibling_product_is_not_answered_with_this_one():
    ...
    found, _ = candidates_from_instance(
        INSTANCE, product="Tyvaso DPI", register=REGISTER)
...
def test_a_product_the_register_does_not_place_yields_nothing():
    found, notes = candidates_from_instance(
        INSTANCE, product="Remodulin", register=REGISTER)
    assert found == []
...
def test_the_same_figure_tagged_twice_is_one_candidate():
    ...
    found, _ = candidates_from_instance(twice, product="Nebulized Tyvaso", register=REGISTER)
    assert len(found) == 1
