...
SOURCE = "Kisqali 956 627 52 56 and other text that the filing contains"


def test_a_string_span_is_read_as_one():
    got = _filter_hallucinated_spans(["Kisqali 956 627 52 56"], SOURCE)
    assert [s["span_text"] for s in got] == ["Kisqali 956 627 52 56"]
    assert got[0]["span_id"] == "s1"


def test_objects_still_work_and_keep_their_fields():
    got = _filter_hallucinated_spans(
        [{"span_id": "x9", "span_text": "Kisqali 956 627 52 56", "why_relevant": "sales"}],
        SOURCE,
    )
...
def test_a_mixed_list_survives():
    got = _filter_hallucinated_spans(
        ["Kisqali 956 627 52 56", {"span_text": "other text that the filing contains"}],
        SOURCE,
    )
    assert len(got) == 2
...
def test_the_verbatim_rule_still_applies_to_a_string_span():
    ...
    assert _filter_hallucinated_spans(["Kisqali 9,999 invented"], SOURCE) == []
