"""The grammar-driven grid reader. Degraded mode only; never scored.

Every rule here decides what a row or a section means from its label
vocabulary. The scored path reads meaning from the fingerprint description
instead (``app.extraction.described``).
"""
...
from app.extraction.columns import Alignment, ColumnLayout, align_row
from app.extraction.degraded.header_grammar import (
    _GEO_HEAD_RE,
    build_layouts,
    split_geography,
)
from app.extraction.fingerprint import detect_currency, detect_unit
from app.extraction.readers import (
    Observation,
    ReadReport,
    _is_data_row,
    _period_bounds,
